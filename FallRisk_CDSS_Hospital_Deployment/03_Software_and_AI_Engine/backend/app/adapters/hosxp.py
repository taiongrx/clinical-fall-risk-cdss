...
class HOSxPAdapter(BaseHISAdapter):
    """
    Production-grade HOSxP v3/v4 Adapter.
    Adheres to:
      - paranoid-db-guard: Connection pooling with pool_pre_ping and timeout <= 10s.
      - his-read-replica-guard: Read-only transactions, never locking production tables.
    """
    def __init__(self):
        self._engine = None
    ...
    def get_engine(self):
        ...
        try:
            ...
            print(f"[HOSxPAdapter] Could not initialize HOSxP engine: {e}")
            return None
    ...
    def get_latest_visit_date(self) -> str:
        ...
        if engine:
            try:
                ...
                print(f"[HOSxPAdapter] Error getting latest vstdate: {e}")
    ...
    def fetch_elderly_visits_by_date_range(
        ...
        try:
            ...
            print(f"[HOSxPAdapter] fetch_elderly_visits_by_date_range failed: {e}")
            return pd.DataFrame()
    ...
    def fetch_patient_clinical_dossier(
        ...
        try:
            with engine.connect() as conn:
                ...
                """
                visit_df = pd.read_sql(query_visit, conn, params={'hn_param': hn, 'index_date_param': index_date_str})
                ...
                if not med_df.empty:
                    ...
                    atc_codes, frid_groups, atc_descs = [], [], []
                    for _, r in med_df.iterrows():
                        atc, grp, desc = tag_drug_atc(
                            ...
                            tmt_code=str(r.get('tmt_tp_code') or r.get('tmt_gp_code') or '')
                        )
                ...
                # 3. Diagnoses
                query_diag = """
                    SELECT vs.hn, vs.vstdate AS prior_diag_date, 
                ...
                """
                diag_df = pd.read_sql(query_diag, conn, params={'hn_param': hn, 'start_date_param': diag_history_start_date, 'end_date_param': index_date_str})
                ...
                try:
                    query_v = """
                        ...
                        ORDER BY vstdate DESC LIMIT 6
                    """
                    df_v = pd.read_sql(query_v, conn, params={'hn_param': hn, 'index_date_param': index_date_str})
                    if not df_v.empty:
                        ...
                        if vitals_latest.get('bps') and vitals_latest['bps'] <= 105:
                            clinical_alerts.append(f"ความดันโลหิตต่ำ (BP {int(vitals_latest['bps'])}/{int(vitals_latest.get('bpd', 0))} mmHg): ระวังอาการหน้ามืด/วิงเวียนขณะลุกยืน")
                        if vitals_latest.get('bmi') and vitals_latest['bmi'] < 18.5:
                            clinical_alerts.append(f"ดัชนีมวลกายต่ำกว่าเกณฑ์ (BMI {vitals_latest['bmi']} kg/m²): เสี่ยงภาวะมวลกล้ามเนื้อน้อยและเปราะบาง")
                except Exception as ev:
                    print(f"[HOSxPAdapter] vitals fetch error for HN {hn}: {ev}")

                # 5. Longitudinal Lab Trends
                try:
                    lab_start = (index_date - timedelta(days=365*2)).strftime('%Y-%m-%d')
                    query_l = """
                        ...
                        WHERE lh.hn = %(hn_param)s AND lh.order_date >= %(lab_start_param)s
                          AND lo.lab_order_result IS NOT NULL AND lo.lab_order_result != ''
                        ORDER BY lh.order_date DESC LIMIT 20
                    """
                    df_l = pd.read_sql(query_l, conn, params={'hn_param': hn, 'lab_start_param': lab_start})
                    if not df_l.empty:
                        ...
                        for _, lrow in df_l.iterrows():
                            ...
                            try:
                                ...
                                if 'egfr' in lname and lval < 60:
                                    a = f"การทำงานของไตลดลง (eGFR {lval} mL/min): ชะลอการกำจัดยา FRIDs ระวังยาตกค้างสะสม"
                                    if a not in clinical_alerts: clinical_alerts.append(a)
                                if ('potassium' in lname or 'k+' in lname) and lval < 3.5:
                                    a = f"เกลือแร่โพแทสเซียมต่ำ (K+ {lval} mmol/L): เสี่ยงกล้ามเนื้ออ่อนแรงและเสียการทรงตัว"
                                    if a not in clinical_alerts: clinical_alerts.append(a)
                            except Exception:
                                continue
                except Exception as el:
                    print(f"[HOSxPAdapter] lab fetch error for HN {hn}: {el}")

                return visit_df, med_df, diag_df, vitals_latest, vitals_trend, lab_trends, clinical_alerts
        except Exception as e:
            print(f"[HOSxPAdapter] fetch_patient_clinical_dossier failed: {e}")
            return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), {}, [], [], []
    ...
    def fetch_fall_incidents(self, start_date: str = '2026-01-01') -> pd.DataFrame:
        ...
        try:
            ...
            print(f"[HOSxPAdapter] fetch_fall_incidents failed: {e}")
            return pd.DataFrame()
